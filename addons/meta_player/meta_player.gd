...
## Audio files to be played. Allows multiple audio files to be mixed at once, for example
## fading out a low intensity track for a high intensity one. Use multiple sync groups to randomise
## which group is played, or play in sequence.
@export var sync_groups : Array[AudioStreamSynchronized]
## Whether to randomise the sync_group, or play in sequence.
@export_enum("Random", "Sequence") var play_mode := "Sequence"
...
@export_range(0.01,3.0) var seek_down_weight := 0.8 ## How quickly the volume should fall, in decibels per frame.
...
var block_automation : bool = false
...
func _process(delta):
	calc_beat(delta)
	if automate_volume and target and !block_automation:
		if !copy: return
		if copy.playing:
			...
			if param_smooth != 1.0:
				...
				param = t_param
			match automation_rule.fade_type:
				"curve":
					fade_to(get_curve_vol(automation_rule.curve))
				"sync":
					group_fade()
...
func get_curve_vol(vol_curve : Curve):
	var vol : float = vol_curve.sample(param)
	vol = (vol*70) - 70
	vol = clamp(vol,-70,0)
	return vol
	
func get_range_vol(sync := false, set_rule : fade_rule = null) -> float:
	var vol : float = param
	var rule : range_fade = automation_rule if !sync else set_rule
...
func get_dis_vol(sync := false, set_rule : fade_rule = null) -> float:
	var rule : point_fade = automation_rule if !sync else set_rule
...
func group_fade(instant := false):
	var sample : AudioStreamSynchronized = stream
	for index in automation_rule.fade_rules.size():
		var rule : curve_fade = automation_rule.fade_rules[index]
		var vol_string : String = "stream_" + str(index) + "/volume"
		var vol : float = sample.get(vol_string)
		var new_vol := 0.0
		new_vol = get_curve_vol(rule.curve)
		if instant:
			vol = new_vol
		else:
			var above := (vol < new_vol) 
			var sw = seek_down_weight
			if above:
				sw = seek_up_weight
			vol = move_toward(vol, new_vol, sw)
		sample.set(vol_string, vol)
		if copy:
			copy.stream.set(vol_string, vol)
...
func set_sync_stream(trial := false):
	if trial:
		stream = sync_groups[0]
		return
	var sample : AudioStreamSynchronized
	if sync_groups.size() == 1:
		stream = sync_groups[0]
	else:
		if play_mode == "Sequence":
			stream = sync_groups[sequence_index]
			sequence_index = wrapi(sequence_index + 1, 0, sync_groups.size())
		else:
			stream = sync_groups[randi() % sync_groups.size()]
...
func mstop():
	...
	if has_play_group:
		stop_group()
	while copy.volume_linear > 0.0:
		copy.volume_linear = move_toward(copy.volume_linear, 0.0, 0.001)
	copy.queue_free()
